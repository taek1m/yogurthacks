...
export function CreateTopicModal() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (value.length < 3) return setError("Enter a topic with at least 3 characters.");
  ...
  return (
    ...
    >
      <form onSubmit={submit} className="p-5 sm:p-6">
        ...
        <label htmlFor="agent-topic" className="mt-6 block text-sm font-bold text-[#294a35]">Agent topic</label>
        <input
          ref={input}
          id="agent-topic"
          value={topic}
          ...
          className="mt-2 h-12 w-full rounded-md border border-[#b7cbb5] bg-white px-3 text-base outline-none focus:border-[#4f865e] focus:ring-2 focus:ring-[#c4dfc9]"
        />
        <p className="mt-2 text-xs leading-5 text-[#65796b]">Gemini will name the agent, choose its garden object, and prepare its first conversation.</p>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button type="submit" disabled={topic.trim().length < 3 || creating} className="inline-flex h-10 items-center gap-2 rounded-md bg-[#225f3b] px-4 text-sm font-semibold text-white hover:bg-[#184b2d] disabled:cursor-not-allowed disabled:opacity-50">
            {creating && <LoaderCircle size={17} className="animate-spin" />}
            {creating ? "Growing agent..." : "Create agent"}
          </button>
        </div>
      </form>
    </dialog>
