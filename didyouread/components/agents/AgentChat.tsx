...
export function AgentChat({ agent, highlightedMessage }: { agent: DocumentAgent; highlightedMessage?: string }) {
  ...
  return (
    <section className="flex min-h-[calc(100vh-4rem)] min-w-0 flex-col bg-[#fffef9] lg:border-r lg:border-[#dce5d9]" aria-labelledby="agent-title">
      ...
      <form onSubmit={ask} className="sticky bottom-0 border-t border-[#dce5d9] bg-[#fffef9] p-4 sm:px-8">
        ...
        <div className="flex items-end gap-2 rounded-lg border border-[#bdcdb9] bg-white p-2 shadow-sm focus-within:border-[#5b9069] focus-within:ring-2 focus-within:ring-[#c4dfc9]">
          <textarea id="agent-question" ref={input} value={question} onChange={(event) => setQuestion(event.target.value)} rows={2} placeholder={agent.sourceKind === "topic" ? `Ask about ${agent.topic || "this topic"}...` : "Ask about a deadline, fee, or clause..."} className="max-h-32 min-h-12 flex-1 resize-none bg-transparent px-2 py-1 text-sm outline-none" />
